Prediction_ParameterEstimation_Location_Train <- function(freqIdx_, locIdx_, state, locLag, model, optimalTuning = NULL){
  ##################
  ## Import data
  ##################
  ...
  if(model == "OLS"){
    ...
    AdLPred <- glmnet_model(train_input = as.matrix(state_x), 
                            train_output = as.data.frame(state_y),
                            val_input = as.matrix(state_x), 
                            lam = optimalTuning, test = TRUE, al = 1,  panelty_factor = 1/abs(betals))
    return(AdLPred)
  }else if(model == 'MC+'){
    paneltyType <- 'MCP' 
    ...
    form <- paste0("Y~s(V1,k=", optimalTuning,",bs =", bs, ")")
    for(idx_ in 2:((locLag+1) * 6)){
      form <- paste0(form, "+s(V", idx_, ",k=",optimalTuning,", bs =", bs, ")")
    }
    form <- as.formula(form)
    
    fit <- gam(form,data=datC)
    colnames(state_x) <- paste0("V", 1:6*(locLag+1))
    predSspline <- predict(fit, state_x)
    ...
    datC <- cbind(state_y,state_x)
    colnames(datC)[1] <- "Y"
    fit <- earth(Y~., datC, degree = optimalTuning)
    predSpline <- predict(fit, state_x)
