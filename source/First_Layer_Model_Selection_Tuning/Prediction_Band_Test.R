###################################################
#
# Create frequency band new testing input variable
#
###################################################
...
source('source/Source_First_Layer_Model_Selection_Tuning/Prediction_ParameterEstimation_Band.R')
...
foreach(freqIdx_=1:8, .options.snow = opts, .packages = c('glmnet', 'data.table')) %dopar% {
  ...
  for(bandIdx_ in 1:6){
    stateNewInput <- NULL
    activatedIdxPath <- paste0('data/HMM_State_Test/activatedIdx_',bandIdx_,'.csv')
    activatedIdx <- read.table(activatedIdxPath, sep = ',')
    inhibitedIdxPath <- paste0('data/HMM_State_Test/inhabitedIdx_',bandIdx_,'.csv')
    inhibitedIdx <- read.table(inhibitedIdxPath, sep = ',')
    stateNewInputIdx <- rbind(activatedIdx, inhibitedIdx)
    for(state_ in 1:2){
      if(state_ == 1){
        predictedValue <- as.data.frame(Prediction_ParameterEstimation_Band(freqIdx_ = freqIdx_, bandIdx_ = bandIdx_, state = stateList[state_], bandLag = 2, model = Optimal_Model_activated[bandIdx_,freqIdx_], optimalTuning = Optimal_Parameter_activated[bandIdx_,freqIdx_]))
        stateNewInput <- rbind(stateNewInput, predictedValue)
      }else if(state_ == 2){
        predictedValue <- as.data.frame(Prediction_ParameterEstimation_Band(freqIdx_ = freqIdx_, bandIdx_ = bandIdx_, state = stateList[state_], bandLag = 2, model = Optimal_model_inhabited[bandIdx_,freqIdx_], optimalTuning = Optimal_Parameter_inhabited[bandIdx_,freqIdx_]))
  ...
  NewInputPath <-  paste0("data/NewVariable/OLS/Test/Freq",freqIdx_,"_Band.csv")
